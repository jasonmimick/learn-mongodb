...
public class JasonMain {
    ...
    private static void bulkAssign(DBCollection coll, int total, int batchSize, int delay) {
        ...
        // insert batch# of docs with int _id - we'll then update
        // these all in batches.
        BulkWriteOperation initialBulk = coll.initializeUnorderedBulkOperation();
        for (int ii=0;ii<batchSize;ii++) {
            initialBulk.insert( new BasicDBObject("_id", ii) );
        }

        BulkWriteResult initialResult = initialBulk.execute();
        int initialDocs = initialResult.getInsertedCount();
        System.out.println("Initial load of " + initialDocs + " docs complete");
        while (i < total) {
            //bulk Update prep
            DBObject findObject;
            ...
            //if (findSize > 0) {
                ...
                for (int x = 0; x <= batchSize; x++) {
                    BasicDBObject fo = new BasicDBObject("_id",x);
                    builder.find( fo ).updateOne(modifyObject);
                }
                ...
                System.out.println("resultSize=" + resultSize + " i=" + i);
            ...
            try {
                ...
                e.printStackTrace();
                BulkWriteException be = (BulkWriteException) e;
                for(BulkWriteError error : be.getWriteErrors()) {
                    System.out.println("BulkWriteError: " + error.getMessage());
                }
